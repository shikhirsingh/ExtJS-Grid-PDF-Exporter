...
folder are mapped to the application's root namespace, 'GridExportPdf'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "GridExportPdf"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'GridExportPdf' in the same way
as `"GridExportPdf/sass/src"`.
